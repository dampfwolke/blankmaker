...
class StatsAuswertung(qtw.QMainWindow, Ui_frm_stats_auswertung):
    ...
    def __init__(self):
        ...
        self.tv_zeit.setModel(self.model)

        # CSV Zeitersparnis laden
    ...
    def csv_laden_zeit(self, file):
        ...
        try:
            ...
            if self.model.columnCount() > 0:
                ...
                header_view.setSectionResizeMode(self.model.columnCount() - 1, qtw.QHeaderView.Stretch)

        except FileNotFoundError:
            print(f"Fehler: Die Datei {filepath} wurde nicht gefunden.")
            # Optional: Fehlermeldung in der GUI anzeigen
            self.model.setHorizontalHeaderLabels(["Fehler"])
            self.model.appendRow([qtg.QStandardItem("CSV-Datei nicht gefunden.")])
    ...
    def auswerten_zeit(self):
        ...
        for row in range(self.model.rowCount()):
            item = self.model.item(row, spalten_index)
